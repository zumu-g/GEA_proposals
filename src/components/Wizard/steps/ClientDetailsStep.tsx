...
// ─── AI-drafted agent copy ───────────────────────────────────────────────────
// Drafts the "your agent" bio and the vendor-specific introduction for this
// proposal. Both are editable: nothing reaches a vendor without the agent
// reading it. Leaving them blank keeps the agency default bio and the generic
// introduction, so this is purely additive.

function AgentCopyPanel({
  formData,
  propertyType,
  onChange,
}: {
  formData: ClientDetailsStepProps['formData']
  propertyType?: PropertyType
  onChange: (field: string, value: any) => void
}) {
  ...
  const canGenerate = !!formData.propertyAddress.trim() && !generating

  const generate = async () => {
    setGenerating(true)
    setError('')
    try {
      const res = await fetch('/api/generate-agent-copy', {
        method: 'POST',
        ...
        body: JSON.stringify({
          ...
          priceGuideMin: formData.priceGuideMin,
          priceGuideMax: formData.priceGuideMax,
        }),
      })
      const data = await res.json()
      if (!res.ok) throw new Error(data.error || 'Generation failed')
      onChange('agentBio', data.bio)
      onChange('introText', data.intro)
      setOpen(true)
    } catch (e) {
  ...
  const hasCopy = !!(formData.agentBio || formData.introText)

  return (
    <div className="mb-6 rounded-lg border border-gray-200 bg-gray-50 p-5">
      <div className="flex items-start justify-between gap-4">
        <div>
          <p className="font-sans text-sm font-medium text-gray-700 lowercase">
            agent intro &amp; bio <span className="text-gray-400 normal-case">(optional)</span>
          </p>
          <p className="text-gray-500 font-sans text-xs mt-1">
            draft copy for this owner and property. leave blank to use your standard bio.
          </p>
        </div>
        <button
          type="button"
          ...
          className="flex-shrink-0 px-4 py-2 rounded-lg bg-[#C41E2A] text-white font-sans text-xs font-medium hover:bg-[#a81824] disabled:opacity-40 transition-all min-h-[36px] flex items-center gap-2"
        >
          {generating ? (
            <>
      ...
      {error && <p className="text-red-600 font-sans text-xs mt-3">{error}</p>}

      {!formData.propertyAddress.trim() && (
        <p className="text-gray-400 font-sans text-xs mt-3">enter the property address first</p>
      )}

      {(hasCopy || open) && (
        <div className="mt-4 space-y-4">
          <div>
            <label htmlFor="introText" className="block font-sans text-xs text-gray-500 lowercase mb-1.5">
              introduction — shown at the top of the proposal
            </label>
            <textarea
              id="introText"
              rows={4}
              ...
              className="w-full rounded-lg border border-gray-300 bg-white px-4 py-3 text-gray-900 font-sans text-sm placeholder-gray-400 focus:ring-2 focus:ring-[#C41E2A] focus:border-[#C41E2A] transition-all"
              placeholder="Leave blank for the standard introduction."
            />
          </div>
          <div>
            <label htmlFor="agentBio" className="block font-sans text-xs text-gray-500 lowercase mb-1.5">
              agent bio — shown in the &quot;your agent&quot; section
            </label>
            <textarea
              id="agentBio"
              rows={4}
              value={formData.agentBio || ''}
              onChange={e => onChange('agentBio', e.target.value)}
              className="w-full rounded-lg border border-gray-300 bg-white px-4 py-3 text-gray-900 font-sans text-sm placeholder-gray-400 focus:ring-2 focus:ring-[#C41E2A] focus:border-[#C41E2A] transition-all"
              placeholder="Leave blank for your standard bio."
            />
          </div>
          <p className="text-gray-400 font-sans text-xs">
            read both before sending — AI drafts can get details wrong.
          </p>
        </div>
      )}
    </div>
  ...
  return (
    <div>
      ...
      >
        ...
        <AgentCopyPanel formData={formData} propertyType={propertyType} onChange={onChange} />
      </motion.div>
