...
export function getComparablesWarning(
  sold: ComparableRow[],
  onMarket: ComparableRow[]
): string | null {
...
/** Extract suburb from a full address like "42 Smith St, Brighton VIC 3186" */
function extractSuburb(address: string): string {
  // Try to find "Suburb STATE POSTCODE" pattern after a comma
  const commaMatch = address.match(/,\s*([A-Za-z\s]+?)(?:\s+[A-Z]{2,3}\s+\d{4})\s*$/)
  if (commaMatch) return commaMatch[1].trim()
  // Try just suburb + postcode
...
/** Check if an address looks complete (has suburb + postcode) */
...
export default function ComparablesStep({
  propertyAddress,
  soldComparables,
  onMarketListings,
  onChangeSold,
  ...
  // Active tab
  const [activeTab, setActiveTab] = useState<'sold' | 'onmarket'>('sold')
  ...
  // Exclusion tracking (persists across re-filters)
    ...
    (sold: any[], onMarket: any[]) => {
      ...
      filteredBuy.sort((a: any, b: any) => {
        ...
        switch (sortBy) {
          ...
          case 'date-desc':
            return 0 // on-market has no date
          case 'beds-desc':
            return (Number(b.bedrooms) || 0) - (Number(a.bedrooms) || 0)
          default:
            return 0
      ...
      setStatusMessage(
        `Found ${soldCount} sold${soldCount < sold.length ? ` (from ${sold.length})` : ''} + ${buyCount} on market${buyCount < onMarket.length ? ` (from ${onMarket.length})` : ''}`
      )
    },
  ...
  }, [distanceFilter, bedsMin, bathsMin, priceMin, priceMax, propType, soldWithin, sortBy, subjectLat, subjectLng, rawComps, rawOnMarket, applyFilters])
    ...
    async (searchAddress?: string) => {
      ...
      try {
        const [soldRes, buyRes] = await Promise.all([
          fetch(`/api/comparables?address=${encodeURIComponent(suburb)}`),
          fetch(`/api/comparables?address=${encodeURIComponent(suburb)}&type=buy`),
        ])
        const [soldData, buyData] = await Promise.all([soldRes.json(), buyRes.json()])

        if (soldData.error && buyData.error) {
        ...
        if (sold.length === 0 && onMarketResult.length === 0) {
          setStatusMessage(
            `No properties found for "${suburb}". Try a different address.`
          )
        } else {
          applyFilters(sold, onMarketResult)
        }
      } catch (err) {
        setStatusMessage(
  ...
  const refreshComparables = useCallback(async () => {
    ...
    try {
      const [soldRes, buyRes] = await Promise.all([
        fetch(`/api/comparables?address=${encodeURIComponent(suburb)}&refresh=true`),
        fetch(`/api/comparables?address=${encodeURIComponent(suburb)}&type=buy&refresh=true`),
      ])
      const [soldData, buyData] = await Promise.all([soldRes.json(), buyRes.json()])
      ...
      if (sold.length === 0 && onMarketResult.length === 0) {
        setStatusMessage(`No properties found on refresh.`)
      } else {
        applyFilters(sold, onMarketResult)
      }
    } catch (err) {
  ...
  useEffect(() => {
    if (confirmedAddress && confirmedAddress !== prevConfirmedRef.current) {
      ...
      // Only auto-search if we don't already have results or this is a new address
      if (compRows.length === 0 && onMarketRows.length === 0) {
        searchComparables(confirmedAddress)
      } else if (hasSearchedRef.current) {
        // Address changed after initial load — re-search
        searchComparables(confirmedAddress)
      }
    }
      ...
      <AnimatePresence>
        {confirmedAddress && (
          ...
          >
              ...
              <div className="p-5 pb-4">
                <div className="flex items-center justify-between mb-3">
                  ...
                  <div className="flex items-center gap-2">
                    {/* Refresh */}
                    {(rawComps.length > 0 || rawOnMarket.length > 0) && (
                      <button
                        type="button"
            ...
            <div className="bg-white border border-gray-200 rounded-xl overflow-hidden">
              ...
              <div className="flex border-b border-gray-200">
                <button
                  ...
                  className={`flex-1 px-5 py-3.5 font-sans text-sm font-medium transition-all relative ${
                    activeTab === 'sold'
                ...
                >
                  ...
                  {activeTab === 'sold' && (
                    <motion.div
                      layoutId="compTabIndicator"
                      className="absolute bottom-0 left-0 right-0 h-0.5 bg-[#C41E2A]"
                              ...
                              <div className="flex-1 min-w-0">
                                <div className="flex items-start justify-between gap-3">
                                  ...
                                  <div className="text-right flex-shrink-0">
                                    <p className="text-[#C41E2A] font-sans text-base font-semibold">
                                      {fmtPrice(row.price)}
                                    </p>
                                  </div>
                                </div>
            ...
            {(selectedSoldCount > 0 || selectedOnMarketCount > 0) && (
              <motion.div
              ...
              >
                <div className="flex items-center justify-between mb-3">
                  ...
                  <p className="text-gray-500 font-sans text-xs">
                    {selectedSoldCount} sold &middot; {selectedOnMarketCount} on market
                  </p>
                </div>
                ...
                {selectedSoldCount > 0 && (
                  <div className="mb-2">
                    <p className="text-gray-500 font-sans text-[10px] uppercase tracking-wider mb-1.5">
                      sold
                    </p>
